...
import {
  clearSettingsFromDom, onSettingsChanged, readSettings, restoreSettings,
  type PixelSkinSettings,
} from './settings.js'
...
function pixelFaviconSvg(fill: string): string {
 ...
 */
export function apply(ctx: PixelSkinContext): void {
  ctx.effect(() => {
    let settings = readSettings()
    let enabled = !settings.enabled
    let disposeTokens: () => void = () => {}
    let disposePalette: () => void = () => {}
    let disposeFavicon: () => void = () => {}
    ...
    const offSettings = onSettingsChanged((next: PixelSkinSettings) => {
      settings = next
